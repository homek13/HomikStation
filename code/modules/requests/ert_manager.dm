...
ADMIN_VERB(ert_manager, R_ADMIN, "ERT Manager", "Manage ERT requests.", ADMIN_CATEGORY_GAME)
	...
	switch(action)
		...
		if("dispatchErt")
			...
			if(should_be_announced)
				priority_announce("Attention, [station_name()]. Your ERT request has been authorized. Resources are being assessed. Results will be announced within a few minutes.", "ERT Protocol Activated")
		...
		if("denyErt")
			GLOB.ert_request_answered = TRUE
			var/message = "[station_name()], we are informing you that ERT deployment is currently unavailable."
			if(params["reason"])
				message += " Your ERT request has been denied for the following reason:\n\n[params["reason"]]"
	...
	if(ert_type == "Blue")
		for(var/role in 1 to slots_sans_leader)
			...
			if(janitor_slots > 0)
				...
				janitor_slots--
			if(chaplain_slots > 0)
				roles.Add(/datum/antagonist/ert/chaplain)
				chaplain_slots--
			if(clown_slots > 0)
				roles.Add(/datum/antagonist/ert/clown)
				clown_slots--
	if(ert_type == "Red")
		for(var/role in 1 to slots_sans_leader)
			if(security_slots > 0)
			...
			if(engineering_slots > 0)
				...
				engineering_slots--
			if(janitor_slots > 0)
				roles.Add(/datum/antagonist/ert/janitor/heavy)
				janitor_slots--
...
/datum/ert_manager/proc/makeERTFromSlots(datum/ert/ert_type_path, admin_slots, commander_slots, security_slots, medical_slots, engineering_slots, janitor_slots, chaplain_slots, clown_slots)
	...
	var/spawn_success = user_holder.make_emergency_response_team(ertemplate, skip_preference_menu = TRUE)

	if(!spawn_success)
		if(should_be_announced)
			minor_announce("[station_name()], we are informing you that ERT deployment is currently unavailable.", "ERT Unavailable")
		return FALSE

	if(should_be_announced)
		var/list/team_adj = list("Blue", "Red", "Inquisition")
		var/list/team_code = list("Blue", "Red", "Inquisition")
		var/announcement = "Attention, [station_name()]. We are dispatching a [team_adj[ert_type] || "special"] Emergency Response Team, code \"[team_code[ert_type] || "UNKNOWN"]\". Stand by."
		priority_announce(announcement, "ERT En Route")
